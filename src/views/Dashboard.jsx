...
const Dashboard = ({ user, apiFetch }) => {
  ...
  const nutritionData = latestMeal ? [
    { name: isEn ? 'Grains' : '全穀雜糧', value: Number(latestMeal.grain_calories) || 0, color: '#f59e0b' },
    { name: isEn ? 'Protein' : '豆魚蛋肉', value: Number(latestMeal.protein_meat_calories) || 0, color: '#ef4444' },
    { name: isEn ? 'Vegetables' : '蔬菜', value: Number(latestMeal.vegetable_calories) || 0, color: '#10b981' },
  ].filter(item => item.value > 0) : [];

  const getRadarData = (rec) => [
    { subject: isEn ? 'Grains' : '全穀雜糧', A: rec?.grain_area || 0 },
    { subject: isEn ? 'Protein' : '豆魚蛋肉', A: rec?.protein_meat_area || 0 },
    { subject: isEn ? 'Vegetables' : '蔬菜', A: rec?.vegetable_area || 0 },
    { subject: isEn ? 'Fruits' : '水果', A: rec?.fruit_area || 0 },
    { subject: isEn ? 'Dairy' : '乳品', A: rec?.dairy_area || 0 },
    { subject: isEn ? 'Nuts & Seeds' : '堅果種子', A: rec?.nuts_area || 0 },
  ];

  const steps = isEn
    ? [
        { title: 'Profile Setup', content: 'Set your height, weight, and goals to calculate calorie needs.', position: 'bottom' },
        { title: 'Meal Logging', content: 'Add meals, upload food photos, and add notes for easy tracking.', position: 'bottom' },
        { title: 'Analytics Overview', content: 'Track calories and macronutrient ratios to see your diet balance.', position: 'top' },
        { title: 'AI Assistant', content: 'Ask the AI diet consultant for instant personalized advice.', position: 'top' },
        { title: 'History Tracking', content: 'Review previous meals and monitor your progress over time.', position: 'none' },
      ]
    : [
  ...
  const getStatusLabel = (status) => {
    if (!isEn) return status.label;
    if (status.tone === 'online') return 'Online';
    if (status.tone === 'checking') return 'Checking...';
    return status.label === '檢查失敗' ? 'Check Failed' : 'Offline';
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-24 px-4 sm:px-6 relative">
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4">
          <div className="bg-white rounded-[40px] w-full max-w-md overflow-hidden border-4 border-amber-400 shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div className="p-8">
              ...
              >
                {isEn ? 'Got it, start using' : '知道了，開始使用'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className={`absolute inset-x-0 ${steps[guideStep].position === 'top' ? 'top-0' : 'bottom-0'} flex justify-center p-6 sm:p-10 pointer-events-none`}>
            <div className="bg-white rounded-[40px] p-8 max-w-xl w-full shadow-2xl border-4 border-emerald-500 pointer-events-auto animate-in zoom-in-95">
              <div className="flex justify-between items-center mb-4 text-emerald-700 font-black">
                <span>{isEn ? `Step ${guideStep + 1} / ${steps.length}` : `步驟 ${guideStep + 1} / ${steps.length}`}</span>
              ...
              <div className="flex gap-4">
                <button onClick={prevStep} disabled={guideStep === 0} className="flex-1 py-4 rounded-2xl font-black border-2 border-slate-200 text-slate-400">{isEn ? 'Previous' : '上一步'}</button>
                <button onClick={nextStep} className="flex-1 py-4 rounded-2xl font-black bg-emerald-600 text-white border-b-4 border-emerald-800 active:border-b-0 active:translate-y-1">{isEn ? 'Next' : '下一步'}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between pt-2">
        <h1 className="text-xl font-black text-slate-800 flex items-center gap-2">{isEn ? 'Dashboard Overview' : '儀表板總覽'} <span className="px-2 py-0.5 bg-amber-400 text-amber-950 text-[10px] rounded-lg border-2 border-amber-600 font-black shadow-[2px_2px_0px_rgba(0,0,0,0.1)] uppercase">Beta</span></h1>
        <button onClick={() => setGuideStep(0)} className="flex items-center gap-2 bg-white border-4 border-slate-100 px-4 py-2 rounded-2xl font-black text-slate-600 hover:border-emerald-400 transition-all active:scale-95 shadow-sm">
          <HelpCircle size={20} className="text-emerald-500" /> <span className="hidden sm:inline">{isEn ? 'Tour Guide' : '新手導覽'}</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div id="guide-bmi" className={`group lg:col-span-1 rounded-[32px] sm:rounded-[40px] border-4 border-emerald-400 relative overflow-hidden shadow-lg transition-all duration-500 ease-out ${guideStep === 1 ? 'z-[310] ring-[12px] ring-emerald-500/30 scale-105 shadow-2xl' : ''}`}>
          <div className="bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 p-8 sm:p-12 text-white relative overflow-hidden">
            ...
            <h1 className="text-3xl sm:text-4xl font-black mb-2">{isEn ? `Welcome back, ${user?.nickname || 'User'}!` : `歡迎回來，${user?.nickname || '使用者'}！`}</h1>
            <p className="text-emerald-50 font-bold max-w-md italic">{isEn ? 'Keep up your healthy rhythm today! Here is your diet & health overview.' : '今天也一起維持健康節奏，看看你的飲食與分析摘要。'}</p>
          </div>
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-emerald-500 to-teal-700 p-5 flex flex-col justify-center opacity-0 pointer-events-none group-hover:pointer-events-auto group-hover:opacity-100 transition-all duration-500 ease-out z-20 text-white">
            <h4 className="text-lg font-black mb-2 flex items-center gap-2"><Scale size={18} /> {isEn ? 'BMI Body Mass Index' : 'BMI 身體質量指數'}</h4>

            {/* BMI 色帶 */}
            <div className="relative w-full h-3 bg-white/20 rounded-full mb-1 overflow-hidden flex border border-white/30 shrink-0">
              <div className="h-full bg-blue-300 w-[18%]" title={isEn ? "Underweight" : "過輕"} />
              <div className="h-full bg-emerald-400 w-[24%]" title={isEn ? "Normal" : "正常"} />
              <div className="h-full bg-orange-400 w-[28%]" title={isEn ? "Overweight" : "過重"} />
              <div className="h-full bg-rose-500 flex-1" title={isEn ? "Obese" : "肥胖"} />
            </div>
            ...
            <div className="space-y-1 text-xs font-bold flex-1">
              {[
                { label: isEn ? 'Underweight' : '體重過輕', range: '< 18.5', active: bmi < 18.5 },
                { label: isEn ? 'Normal' : '健康體位', range: '18.5 - 24', active: bmi >= 18.5 && bmi < 24 },
                { label: isEn ? 'Overweight' : '體重過重', range: '24 - 27', active: bmi >= 24 && bmi < 27 },
                { label: isEn ? 'Obese' : '肥胖等級', range: '>= 27', active: bmi >= 27 }
              ].map((item, i) => (
        ...
        <div id="guide-bmr" className={`group bg-white p-6 rounded-[32px] border-4 border-amber-400 flex flex-col justify-center relative overflow-hidden transition-all duration-500 ease-out cursor-pointer hover:-translate-y-2 hover:shadow-[0_20px_40px_-15px_rgba(245,158,11,0.4)] ${guideStep === 1 ? 'z-[310] ring-[12px] ring-amber-500/30 scale-105 shadow-2xl' : ''}`}>
          <div className="relative z-10 transition-all duration-500 group-hover:opacity-0 group-hover:-translate-y-6 group-hover:scale-95">
            <h3 className="text-sm font-black text-slate-400 mb-1 flex items-center gap-2"><Flame size={16} className="text-amber-500" /> {isEn ? 'BMR Basal Metabolic Rate' : 'BMR 基礎代謝率'}</h3>
            <div className="flex items-end gap-2 mb-2"><span className="text-4xl font-black text-slate-800">{bmr > 0 ? bmr : '--'}</span><span className="text-sm font-bold text-slate-400 mb-1.5">{isEn ? 'kcal/day' : 'kcal/天'}</span></div>
            <p className="text-xs font-bold text-slate-400">{isEn ? 'Daily calories needed for basic bodily functions' : '身體每天維持基本機能所需熱量'}</p>
          </div>
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-amber-500 to-orange-700 p-8 flex flex-col justify-center opacity-0 pointer-events-none group-hover:pointer-events-auto group-hover:opacity-100 transition-all duration-500 ease-out z-20 text-white">
            <h4 className="text-xl font-black mb-4 flex items-center gap-2"><Flame size={20} /> {isEn ? 'Daily Calorie Advice' : '每日熱量建議'}</h4>

            <div className="bg-white/10 rounded-2xl p-4 mb-6 border border-white/20 backdrop-blur-sm">
              <p className="text-[10px] font-black uppercase opacity-70 mb-2 tracking-widest">{isEn ? 'Calculation Reference' : '計算參考資料'}</p>
              <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-sm font-bold">
                <div className="flex justify-between border-b border-white/10 pb-1"><span>{isEn ? 'Weight' : '體重'}</span><span>{weight}kg</span></div>
                <div className="flex justify-between border-b border-white/10 pb-1"><span>{isEn ? 'Height' : '身高'}</span><span>{height}cm</span></div>
                <div className="flex justify-between border-b border-white/10 pb-1"><span>{isEn ? 'Age' : '年齡'}</span><span>{age}歲</span></div>
                <div className="flex justify-between border-b border-white/10 pb-1"><span>{isEn ? 'Gender' : '性別'}</span><span>{gender === 'Female' || gender === 'female' ? (isEn ? 'Female' : '女性') : (isEn ? 'Male' : '男性')}</span></div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="bg-white text-orange-700 p-4 rounded-2xl shadow-xl transform transition-transform duration-500 translate-y-4 group-hover:translate-y-0">
                <div className="flex justify-between items-center mb-1">
                  <span className="text-xs font-black uppercase">{isEn ? 'Estimated Total Daily Energy Expenditure (TDEE)' : '預估每日總消耗熱量 (TDEE)'}</span>
                  <span className="bg-orange-100 px-2 py-0.5 rounded text-[10px]">{isEn ? 'Light Activity' : '輕度活動'}</span>
                </div>
                <div className="text-2xl font-black">{Math.round(bmr * 1.375)} <span className="text-sm">kcal</span></div>
              </div>

              <p className="text-[11px] font-bold leading-relaxed opacity-90 italic px-2 transform transition-transform duration-500 delay-100 translate-y-4 group-hover:translate-y-0">{isEn ? `We suggest keeping daily calorie intake between ${Math.round(bmr * 1.2)} ~ ${Math.round(bmr * 1.5)} kcal, adjusted for activity level and goals.` : `建議把每日熱量控制在 ${Math.round(bmr * 1.2)} ~ ${Math.round(bmr * 1.5)} kcal，並依活動量與目標逐步微調。`}</p>
            </div>
          </div>
        </div>

        {/* 今日造訪與{isEn ? 'AI Chatbot Status' : '聊天機器人狀態'} */}
        <div className="group bg-slate-900 p-6 rounded-[32px] border-4 border-slate-800 hover:border-emerald-500 flex flex-col justify-center relative overflow-hidden shadow-lg transition-all duration-500 hover:-translate-y-2 cursor-default">
          ...
          <div className="relative z-10 flex flex-col h-full">
            <h3 className="text-sm font-black text-white/50 mb-1 flex justify-between items-center">
              <span className="flex items-center gap-2 transition-colors duration-300 group-hover:text-emerald-400"><Users size={16} className="text-emerald-400" /> {isEn ? 'Today Visits' : '今日造訪人次'}</span>
              <div className="bg-emerald-500/20 border-2 border-emerald-500 text-emerald-400 text-[10px] font-black px-2 py-0.5 rounded-full animate-pulse">{isEn ? 'LIVE' : '即時'}</div>
            </h3>
            <div className="flex items-end gap-2 mb-2 text-white"><span className="text-4xl font-black group-hover:text-emerald-300 transition-colors duration-300">{todayVisit}</span><span className="text-sm font-bold text-slate-400 mb-1.5">{isEn ? 'visits' : '人次'}</span></div>
            <div className="mb-2 flex items-center gap-2 text-xs font-bold">
              ...
              <span className={chatbotStatusColor}>{getStatusLabel(chatbotStatus)}</span>
            </div>
            ...
            >
              ...
              <span className="text-slate-400">{isEn ? 'Gemma4 Model Status' : 'Gemma4 模型狀態'}</span>
              <span className={gemmaStatusColor}>{getStatusLabel(gemmaStatus)}</span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div id="guide-trend" className={`lg:col-span-2 bg-white p-6 sm:p-8 rounded-[32px] border-4 border-slate-100 flex flex-col justify-center relative transition-all duration-300 ${guideStep === 2 ? 'z-[310] ring-[12px] ring-emerald-500/30 scale-[1.01] shadow-2xl bg-white' : ''}`}>
          <h2 className="text-xl font-black text-slate-800 flex items-center gap-2 mb-6"><TrendingUp className="text-emerald-500" /> {isEn ? 'AI Health Score Trend' : 'AI 健康分數趨勢'}</h2>
        ...
        <div id="guide-pie" className={`bg-white p-6 sm:p-8 rounded-[32px] border-4 border-slate-100 flex flex-col transition-all duration-300 ${guideStep === 3 ? 'z-[310] ring-[12px] ring-emerald-500/30 scale-[1.01] shadow-2xl bg-white' : ''}`}>
          <h2 className="text-xl font-black text-slate-800 flex items-center gap-2 mb-2"><PieChartIcon className="text-emerald-500" /> {isEn ? 'Diet Calorie Distribution' : '飲食熱量分布'}</h2>
          <div className="flex-1 flex flex-col items-center justify-center min-h-[300px]">
            {latestMeal ? (
              <>
                <p className="text-xs font-bold text-slate-400 mb-4">{isEn ? 'Total Calories' : '總熱量'} <span className="text-emerald-600 font-black">{latestMeal.total_calories?.toFixed(0)} kcal</span></p>
                <div className="h-64 w-full relative">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-8">
                    ...
                    <span className="text-[10px] font-bold text-slate-400">{isEn ? 'Score' : '分數'}</span>
                  </div>
                </div>
              </>
            ) : <div className="text-slate-300 font-bold">{isEn ? 'No data available' : '目前沒有資料'}</div>}
          </div>
        </div>
      </div>

      <div id="guide-history" className="bg-white p-6 sm:p-8 rounded-[32px] border-4 border-slate-100 shadow-sm">
        <h2 className="text-xl font-black text-slate-800 flex items-center gap-2 mb-6"><Clock className="text-emerald-500" /> {isEn ? 'Recent Diet Records' : '近期飲食紀錄'}</h2>
        <div className="space-y-4">
          {dietRecords.slice(0, 5).map((record, index) => (
            <div key={index} onClick={() => setSelectedRecord(record)} className="bg-white p-5 rounded-2xl border-4 border-slate-200 hover:border-emerald-400 transition-all cursor-pointer flex justify-between items-center group shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div><p className="text-sm font-black text-slate-800 capitalize">{isEn ? 'YOLO Meal Recognition' : 'YOLO 飲食辨識'}</p><p className="text-[10px] font-bold text-slate-400">{safeParseDate(record.created_at).toLocaleString()}</p></div>
              </div>
              <div className="text-sm font-black text-slate-700 border-2 border-slate-100 px-4 py-2 rounded-xl group-hover:border-emerald-400 transition-all">{isEn ? 'Total Calories ' : '總熱量 '}<span className="text-emerald-600">{record.total_calories?.toFixed(0)}</span> kcal</div>
            </div>
          ))}
        </div>
      </div>
          ...
          <div className={`bg-white rounded-[40px] w-full max-w-lg relative border-4 flex flex-col max-h-[90vh] shadow-2xl transition-all ${guideStep === 4 ? 'border-emerald-500' : 'border-slate-100'}`}>
            {guideStep === 4 && (
              <div className="bg-emerald-600 p-6 rounded-t-[36px] text-white animate-in slide-in-from-top-4">
                <div className="flex justify-between items-center mb-3 text-xs font-black uppercase bg-emerald-700/50 w-fit px-2 py-1 rounded">{isEn ? 'Final Tour Step' : '導覽最後一步'}</div>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => { setGuideStep(null); setSelectedRecord(null); }} className="flex-[2] py-3 bg-white text-emerald-700 rounded-xl font-black shadow-lg hover:scale-95 transition-all">{isEn ? 'Complete Tour & Close' : '完成導覽並關閉'}</button>
                </div>
              </div>
            )}
            <div className="p-6 border-b-4 border-slate-100 bg-white rounded-t-[32px]">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-black text-slate-800 flex items-center gap-2"><Activity size={20} className="text-emerald-500" /> {isEn ? 'Record Details' : '紀錄詳細資訊'}</h3>
              ...
              <div className="flex gap-3">
                <button onClick={() => setActiveModalTab('analysis')} className={`flex-1 py-3 rounded-xl border-4 font-black transition-colors ${activeModalTab === 'analysis' ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'bg-slate-50 border-slate-200 text-slate-400'}`}>{isEn ? 'Analysis' : '分析結果'}</button>
                <button onClick={() => handleTabSwitch('image')} className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border-4 font-black transition-colors ${activeModalTab === 'image' ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'bg-slate-50 border-slate-200 text-slate-400'}`}><ImageIcon size={18} /> {isEn ? 'Food Image' : '影像紀錄'}</button>
              </div>
            </div>
              ...
              {activeModalTab === 'analysis' ? (
                <>
                  <div className="bg-slate-50 p-4 rounded-[32px] border-4 border-slate-100 min-h-[300px]">
                    <h4 className="text-center font-black text-slate-700 mb-2">{isEn ? 'Six Food Groups Distribution' : '六大類食物比例'}</h4>
                    <div className="h-64 w-full"><ResponsiveContainer width="100%" height="100%"><RadarChart cx="50%" cy="50%" outerRadius="70%" data={getRadarData(selectedRecord)}><PolarGrid stroke="#cbd5e1" strokeWidth={2} /><PolarAngleAxis dataKey="subject" tick={{ fontSize: 11, fontWeight: 800, fill: '#64748b' }} /><RechartsRadar name={isEn ? "Calories" : "熱量"} dataKey="A" stroke="#10b981" strokeWidth={4} fill="#10b981" fillOpacity={0.4} /></RadarChart></ResponsiveContainer></div>
                  </div>
                  <div className="bg-emerald-50 p-6 rounded-[32px] border-4 border-emerald-100">
                    <div className="flex items-center gap-2 mb-2 text-emerald-700 font-black"><Sparkles size={18} /> {isEn ? 'AI Health Advice' : 'AI 健康建議'}</div>
                    <p className="text-sm font-bold text-emerald-900 leading-relaxed italic">"{selectedRecord?.ai_evaluation || (isEn ? 'AI has not provided feedback yet...' : 'AI 尚未提供評語...')}"</p>
                  </div>
                  <div className="grid grid-cols-2 gap-3 pb-4">
                    <div className="bg-slate-50 p-4 rounded-3xl border-4 border-slate-100 text-center shadow-inner"><p className="text-[10px] font-black text-slate-400 uppercase">{isEn ? 'Total Calories (Est.)' : '總熱量（估算）'}</p><p className="text-xl font-black text-slate-800">{Number(selectedRecord?.total_calories || 0).toFixed(0)} <span className="text-xs">kcal</span></p></div>
                    <div className="bg-slate-50 p-4 rounded-3xl border-4 border-slate-100 text-center shadow-inner"><p className="text-[10px] font-black text-slate-400 uppercase">健康分數</p><p className="text-xl font-black text-emerald-600">{selectedRecord?.ai_health_score} <span className="text-xs">{isEn ? 'pts' : '分'}</span></p></div>
                  </div>
                </>
              ) : (
                <div className="flex flex-col items-center justify-center min-h-[300px] bg-slate-50 rounded-[32px] border-4 border-slate-100 p-4">
                  {imageFetchStatus === 'loading' ? <Activity size={40} className="animate-spin text-emerald-500" /> : recordImageBase64 ? <img src={`data:image/jpeg;base64,${recordImageBase64}`} className="max-w-full rounded-2xl border-4 border-emerald-100 shadow-md" alt="record" /> : <div className="text-slate-400 font-bold">{isEn ? 'No image data available' : '目前無影像資料'}</div>}
                </div>
              )}
            </div>
          </div>
