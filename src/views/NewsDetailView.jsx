...
const NewsDetailView = ({ apiFetch }) => {
    ...
    const fetchDetail = async () => {
      ...
      try {
        ...
        if (NOT_FOUND_ERRORS.has(err?.message) || err?.status === 404) {
          ...
          setError(err?.message || (isEn ? 'Unable to load article' : '無法載入新聞內容'));
        }
      } finally {
  ...
  useEffect(() => {
    const fallbackTitle = isEn ? 'News Detail | Healthy Diet' : '新聞詳情 | Healthy Diet';
    const title = item?.title ? `${item.title} | Healthy Diet` : fallbackTitle;
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      {!loading && error ? (
        <div className="rounded-3xl border border-rose-200 bg-rose-50 px-6 py-5 text-rose-700 shadow-sm">
          <p className="text-lg font-black">{isEn ? 'Failed to Load' : '載入失敗'}</p>
      ...
      {!loading && !error && notFound ? (
        <div className="rounded-[36px] border border-dashed border-slate-300 bg-white px-8 py-14 text-center shadow-sm">
          ...
          <h1 className="mt-5 text-2xl font-black text-slate-900">{t('news.notFoundTitle', '找不到這篇新聞')}</h1>
          <p className="mt-3 text-sm font-medium text-slate-500">
            {t('news.notFoundDesc', '後端回傳 news_not_found 或 news_file_not_found，可以返回列表重新選擇文章。')}
          </p>
        </div>
      ) : null}
          ...
          <section className="overflow-hidden rounded-[36px] border border-slate-200 bg-linear-to-br from-white via-slate-50 to-emerald-50 p-8 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-3 text-sm font-semibold text-slate-500 sm:flex-row sm:items-center sm:justify-between">
                <p>{t('news.publishedDate', '發布日期：')}{item.publishedDate || t('news.dateNotProvided', '未提供')}</p>
                {item.sourceUrl ? (
                  <a
                    href={item.sourceUrl}
                    target="_blank"
