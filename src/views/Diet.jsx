...
import {
  ...
  PieChart as PieChartIcon, HeartPulse, Zap, Info, TrendingUp, ScanLine
} from 'lucide-react';
import {
    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-24">
        ...
        <div className="mb-6 space-y-6">
          {previewUrl ? (
            <div className={`relative border-2 rounded-[32px] p-4 shadow-inner overflow-hidden transition-all duration-500 ${result ? 'bg-emerald-50 border-emerald-200' : 'bg-slate-50 border-slate-200'}`}>
              ...
              <button onClick={clearSelection} className="absolute top-6 right-6 bg-white p-2 rounded-full shadow-md text-red-500 border-2 border-red-50 hover:bg-red-50 hover:scale-110 transition-all">
                <X size={20} />
              </button>
            </div>
          ) : (
            <div className="border-2 border-dashed border-slate-300 rounded-[32px] p-10 text-center bg-slate-50/50 hover:bg-slate-50 transition-colors">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <button onClick={() => document.getElementById('fileUpload').click()} className="bg-white border-2 border-slate-200 py-3 px-6 rounded-xl font-bold flex items-center justify-center hover:bg-slate-50 transition shadow-sm">
                  <ImageIcon size={20} className="mr-2" /> 相簿選擇
                </button>
                <input type="file" id="fileUpload" className="hidden" accept="image/*" onChange={handleFileChange} />
                <button onClick={useTestImage} disabled={isAnalyzing} className="bg-emerald-600 text-white py-3 px-6 rounded-xl font-bold flex items-center justify-center hover:bg-emerald-700 transition shadow-md disabled:opacity-50">
                  <Zap size={20} className="mr-2" /> {isAnalyzing ? '讀取測試圖...' : '使用測試圖片'}
                </button>
              </div>
            </div>
          )}
            ...
            <div className="bg-emerald-50 rounded-[32px] p-6 sm:p-8 border-2 border-emerald-200 shadow-sm animate-in slide-in-from-top-4 duration-500">
              <div className="flex justify-between items-center mb-6 border-b-2 border-emerald-100 pb-4">
                <h3 className="text-lg sm:text-xl font-black text-emerald-800 flex items-center gap-2">
                  <CheckCircle2 className="text-emerald-500" size={24} /> {result.message || '辨識成功'}
                </h3>
                <div className="bg-emerald-600 text-white px-4 py-2 rounded-xl shadow-md border border-emerald-500 flex items-baseline gap-2">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {result.detected_items?.map((item, idx) => (
                  <div key={idx} className="bg-white p-4 rounded-2xl border-2 border-emerald-100 shadow-sm text-center flex flex-col justify-center hover:border-emerald-300 hover:shadow-md transition-all">
                    <div className="bg-emerald-100 text-emerald-700 w-fit mx-auto px-2 py-0.5 rounded text-[10px] font-black uppercase mb-1 border border-emerald-200">
                ...
                {(!result.detected_items || result.detected_items.length === 0) && (
                  <div className="col-span-full text-center text-slate-400 font-bold py-4">未能明確辨識出食物組件。</div>
                )}
              </div>
            </div>
          )}
      ...
      <div className="bg-white rounded-[40px] shadow-sm border-2 border-slate-100 overflow-hidden flex flex-col h-[600px] md:h-[700px]">
        <div className="p-5 border-b-2 border-slate-100 bg-slate-50 flex items-center justify-between">
          ...
          <span className="text-xs font-bold text-slate-400">點擊左側紀錄查看雷達圖詳情</span>
        </div>

        <div className="flex-1 flex overflow-hidden">
          {/* 左側列表 */}
          <div className="w-full md:w-1/3 border-r-2 border-slate-100 overflow-y-auto bg-slate-50/30 scrollbar-hide">
            {records.map((rec, idx) => (
              <div
                key={rec.id || idx}
              ...
              >
                ...
                <p className="text-[11px] text-slate-500 line-clamp-2 leading-relaxed font-medium">{rec.ai_evaluation}</p>
              </div>
            ))}
          </div>

          {/* 右側內容 (電腦版顯示) */}
          <div className="hidden md:flex flex-1 p-8 overflow-y-auto bg-slate-50/50 flex-col gap-8 scrollbar-thin scrollbar-thumb-slate-200">
            {selectedChatRecord ? (
              <>
                <div className="self-end max-w-[90%] w-full animate-in slide-in-from-right-4">
                  <div className="bg-white p-6 rounded-[32px] rounded-tr-none shadow-sm border-2 border-indigo-100">
                ...
                <div className="self-start max-w-[85%] animate-in slide-in-from-left-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="bg-emerald-600 text-white p-5 rounded-[32px] rounded-tl-none shadow-md border-2 border-emerald-500">
          ...
          <div className="bg-white w-full max-h-[90vh] rounded-t-[40px] shadow-2xl overflow-y-auto animate-in slide-in-from-bottom-10 duration-500">
            ...
            <div className="p-6 space-y-8">
              <div className="flex justify-between items-center">
                <div className="bg-indigo-600 text-white px-5 py-2 rounded-2xl shadow-lg border-2 border-indigo-500">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs font-black text-slate-700 bg-slate-100 px-2 py-1 rounded-md border border-slate-200">{selectedChatRecord.created_at?.split('.')[0]}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 pb-10">
                <div className="bg-slate-50 p-4 rounded-3xl border-2 border-slate-200">
                ...
                <div className="bg-slate-50 p-4 rounded-3xl border-2 border-slate-200">
                  ...
                  <p className="text-lg font-black text-emerald-600 flex items-center gap-1"><CheckCircle2 size={16} /> 成功</p>
                </div>
              </div>
            </div>
          </div>
