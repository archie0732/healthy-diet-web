...
const Profile = ({ user, apiFetch, showNotification, fetchProfile, handleLogout }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (isTestAccount) {
      showNotification(t('profile.testAccountWarning', '測試帳號無法修改個人資料！'), 'error');
      return;
    }
    ...
    try {
      ...
      showNotification(t('profile.updateSuccess', '資料更新成功！'));
      fetchProfile();
    } catch (err) {
  ...
  const handleFeedbackSubmit = async () => {
    if (rating === 0) {
      showNotification(isEn ? 'Please select a star rating first' : '請先選擇星級評分', 'error');
      return;
    }
    ...
    try {
      ...
      showNotification(t('profile.feedbackThanks', '感謝您的回饋！'));
      setFeedback('');
    } catch (err) {
      showNotification(err?.message || (isEn ? 'An unexpected error occurred' : '發生預期外的錯誤'), 'error');
    } finally {
  ...
  let bmiMessage = t('profile.bmiEmpty', '請先補全身高與體重數據，以解鎖您的專屬 BMI 分析。');
  if (parseFloat(bmi) > 0) {
    const bmiValue = parseFloat(bmi);
    if (bmiValue < 18.5) bmiMessage = t('profile.bmiUnderweightMsg', '您的體重過輕，建議增加優質蛋白質與碳水化合物的攝取。');
    else if (bmiValue >= 18.5 && bmiValue < 24) bmiMessage = t('profile.bmiNormalMsg', '您的體重處於健康標準區間，請繼續保持目前的飲食與運動規律！');
    else if (bmiValue >= 24 && bmiValue < 27) bmiMessage = t('profile.bmiOverweightMsg', '您的體重稍微過重，建議搭配適度有氧運動與飲食控制。');
    else bmiMessage = t('profile.bmiObeseMsg', '您的體重處於肥胖區間，請多加注意飲食管理，必要時尋求專業協助。');
  }
    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-20 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-[32px] border-2 border-slate-200 shadow-sm flex flex-col items-center">
          <h3 className="font-black text-slate-800 mb-2 flex items-center gap-2">
            <TrendingDown size={18} className="text-indigo-500" /> {t('profile.bmiCardTitle', '身體質量狀態')}
          </h3>
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-[32px] border-2 border-slate-200 shadow-sm flex flex-col justify-center space-y-6">
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                <span className="font-black text-slate-700 flex items-center gap-2">
                  <Droplets className="text-blue-500" size={20} /> {t('profile.waterIntakeGoal', '每日飲水進度')}
                </span>
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                <span className="font-black text-slate-700 flex items-center gap-2">
                  <Dumbbell className="text-emerald-500" size={20} /> {t('profile.exerciseGoal', '本週運動達標')}
                </span>
              ...
              <span className="font-bold text-emerald-600">{exerciseProgress} / 5 {isEn ? 'days' : '天'}</span>
            </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-10 rounded-[40px] border-2 border-slate-200 shadow-sm space-y-8">
          ...
          <div className="bg-rose-50 border-2 border-rose-200 p-4 sm:p-6 rounded-[32px] flex flex-col sm:flex-row items-center justify-between gap-4 mb-4 animate-in slide-in-from-top-4">
            <div className="flex items-center gap-3 text-rose-700">
              ...
              <p className="text-sm font-bold leading-relaxed">
                {isEn ? (
                  <>Demo public test account (<span className="font-black underline">ckck@gmail.com</span>). Profile modification is disabled.</>
                ) : (
                  <>
            ...
            >
              {t('common.viewDetails', '查看詳情')} <ChevronRight size={16} />
            </button>
          </div>
        )}
        ...
        <div className="flex items-center justify-between border-b-2 border-slate-50 pb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black text-slate-800">{t('profile.basicInfo', '基本資料')}</h2>
              <p className="text-slate-500 text-sm font-bold">{t('profile.subtitle', '更新身體參數以維持 AI 辨識的精準度')}</p>
            </div>
          </div>
          <button
            type="submit"
          ...
          >
            <CheckCircle2 size={20} /> {t('profile.saveChanges', '儲存修改')}
          </button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <ProfileInput label={t('profile.nickname', '暱稱')} icon={User} value={formData.nickname} onChange={v => setFormData({ ...formData, nickname: v })} disabled={isTestAccount} />
          <ProfileInput label={t('profile.height', '身高 (cm)')} icon={Ruler} type="number" value={formData.height} onChange={v => setFormData({ ...formData, height: v })} disabled={isTestAccount} />
          <ProfileInput label={t('profile.weight', '體重 (kg)')} icon={Weight} type="number" value={formData.weight} onChange={v => setFormData({ ...formData, weight: v })} disabled={isTestAccount} />
          <ProfileInput label={t('profile.age', '年齡')} icon={Calendar} type="number" value={formData.age} onChange={v => setFormData({ ...formData, age: v })} disabled={isTestAccount} />

          <div className="space-y-2">
            <label className="text-xs font-black text-slate-400 uppercase ml-2 flex items-center gap-1"><Dna size={14} /> {t('profile.gender', '性別')}</label>
            <select
            ...
            >
              <option value="">{isEn ? 'Select gender' : '選擇生理性別'}</option>
              <option value="Male">{t('profile.genderMale', '男性')}</option>
              <option value="Female">{t('profile.genderFemale', '女性')}</option>
            </select>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-xs font-black text-slate-400 uppercase ml-2 flex items-center gap-1"><ShieldAlert size={14} /> {t('profile.healthConditions', '健康狀況 / 慢性疾病')}</label>
            <textarea
              ...
              placeholder={t('profile.healthConditionsPlaceholder', '例如：高血壓、糖尿病...')}
            />
          </div>
          <div className="space-y-2">
            <label className="text-xs font-black text-slate-400 uppercase ml-2 flex items-center gap-1"><Ban size={14} /> {t('profile.dietaryRestrictions', '飲食禁忌 / 過敏原')}</label>
            <textarea
              ...
              rows="3"
              className="w-full px-5 py-4 border-2 border-slate-100 rounded-[32px] bg-slate-50 focus:bg-white focus:border-indigo-500 outline-none font-bold transition-all resize-none disabled:opacity-60"
              placeholder={t('profile.dietaryRestrictionsPlaceholder', '例如：花生過敏、全素...')}
            />
          </div>
        </div>
      </form>
        ...
        {!isFeedbackSubmitted ? (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center gap-4 border-b-2 border-slate-50 pb-6">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="text-xl font-black text-slate-800">{t('profile.feedbackTitle', '意見回報與評分')}</h2>
                ...
                <p className="text-slate-500 text-sm font-bold">{t('profile.feedbackSubtitle', '您的反饋是我們前進的動力')}</p>
              </div>
            </div>

            <div className="flex flex-col items-center py-4 space-y-6">
              <div className="flex flex-col items-center gap-3">
                <p className="text-sm font-black text-slate-400 uppercase">{t('profile.starRatingPrompt', '點擊星等進行評分')}</p>
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      key={star}
              ...
              <div className="w-full space-y-4">
                <div className="space-y-2">
                  <label className="text-xs font-black text-slate-400 uppercase ml-2 flex items-center gap-1">
                    <Sparkles size={14} /> {isEn ? 'Feedback & Suggestions' : '具體建議或心得'}
                  </label>
                  <textarea
                    value={feedback}
                    ...
                    placeholder={t('profile.feedbackPlaceholder', '在這裡寫下您對本系統的任何建議...')}
                  />
                </div>
                ...
                >
                  ...
                  {t('profile.submitFeedback', '送出回饋意見')}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="py-12 flex flex-col items-center text-center animate-in zoom-in-95 duration-500">
            ...
            <h2 className="text-3xl font-black text-slate-800 mb-3">{t('profile.feedbackThanks', '感謝您的回饋！')}</h2>
            <p className="text-slate-500 font-bold max-w-xs mx-auto leading-relaxed">
              {isEn
                ? `Received your ${rating}-star rating! Your suggestions help make Healthy Diet even better.`
                : `收到您的 ${rating} 星好評！您的建議將讓 Healthy Diet 變得更好。`}
            </p>
            ...
            >
              {isEn ? 'Submit another feedback' : '再次填寫意見'}
            </button>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between px-6">
        <Link to="/privacy" className="text-sm font-bold text-slate-400 hover:text-indigo-600 transition-colors flex items-center gap-2">
          <BookOpen size={16} /> {isEn ? 'Browse Privacy Policy' : '瀏覽隱私政策'}
        </Link>
        <button
          type="button"
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white rounded-[40px] w-full max-w-md overflow-hidden border-4 border-slate-100 shadow-2xl animate-in zoom-in-95">
            <div className="bg-slate-900 p-8 text-white relative">
              ...
              <h2 className="text-2xl font-black italic">{isEn ? 'Demo Test Account Terms' : '公共測試帳號說明'}</h2>
            ...
            <div className="p-8 space-y-6">
              <div className="space-y-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="font-black text-slate-800">{isEn ? 'API Testing Purpose' : 'API 測試用途'}</h4>
                    <p className="text-xs font-bold text-slate-500 leading-relaxed">
                      {isEn
                        ? 'This account is dedicated to feature demonstration and API testing. Write and delete operations are restricted.'
                        : '此帳號專供系統功能演示與後端 API 壓力測試使用，部分資料寫入與刪除權限受限。'}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="font-black text-slate-800">{isEn ? 'Data Reset Reminder' : '資料重置提醒'}</h4>
                    <p className="text-xs font-bold text-slate-500 leading-relaxed">
                      {isEn
                        ? 'To ensure environment cleanliness, records in this public account are routinely cleared and not backed up.'
                        : '為確保測試環境整潔，公共帳號內的飲食紀錄與數據將不定期自動清除，且不提供備份服務。'}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="font-black text-slate-800">{isEn ? 'Personal Account Recommended' : '建議註冊個人帳號'}</h4>
                    <p className="text-xs font-bold text-slate-500 leading-relaxed">
                      {isEn
                        ? 'For complete personalized health management and progress graphs, please register a personal account.'
                        : '若需完整體驗個人化健康管理、保存歷史分析曲線，請使用您的專屬信箱註冊個人帳號。'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              >
                {isEn ? 'I Understand the Terms' : '我已瞭解相關條款'}
              </button>
            </div>
          </div>
        </div>
