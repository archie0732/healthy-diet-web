...
const Member = () => {
  ...
  const team = [
    {
      name: 'Arch1e',
      role: t('member.members.arch1e.role', 'Full-Stack Developer'),
      tags: ['React', 'Rust', 'Vite'],
      desc: t('member.members.arch1e.desc', '負責前後端整合、介面設計與部署流程，讓產品從原型一路走到可實際使用的服務。'),
      image: '/team1.webp',
      icon: <Terminal size={14} />,
    },
    {
      name: 'kamiya',
      role: t('member.members.kamiya.role', 'Mobile App Lead'),
      tags: ['Flutter', 'iOS', 'Android'],
      desc: t('member.members.kamiya.desc', '聚焦跨平台體驗與行動端流程設計，讓健康紀錄能在不同裝置上都保持一致與順手。'),
      image: '/team2.webp',
      icon: <Cpu size={14} />,
    },
    {
      name: '空白',
      role: t('member.members.blank.role', 'CV & ML Engineer'),
      tags: ['YOLOv8', 'PyTorch'],
      desc: t('member.members.blank.desc', '處理影像辨識與模型流程，把餐點辨識與資料分析能力接進整體健康飲食體驗。'),
      image: '/team3.webp',
      icon: <Code size={14} />,
    },
    {
      name: '呵公子',
      role: t('member.members.he.role', 'AI Strategy & NLP'),
      tags: ['Prompt Engineering', 'LLM'],
      desc: t('member.members.he.desc', '規劃 AI 對話策略、知識檢索與回答品質，讓系統在提供建議時更可靠也更貼近使用者需求。'),
      image: '/team5.png',
  ...
  const changelog = [
    {
      ...
      date: '2026-04-10',
      content: isEn
        ? 'Added chart analytics experience, optimized image recognition flow, and continuously tuned AI diet interaction.'
        : '新增圖表分析體驗、優化影像辨識流程，並持續調整 AI 飲食建議的互動細節。',
    },
    {
      version: 'v1.1.5',
      date: '2026-04-05',
      content: isEn
        ? 'Enhanced mobile experience and proxy route stability for smoother login, query, and chat.'
        : '改善行動版操作與代理路由穩定性，讓登入、查詢與聊天體驗更順暢。',
    },
    {
      version: 'v1.0.0',
      date: '2026-03-20',
      content: isEn
        ? 'Healthy Diet officially launched, integrating diet tracking, AI dialogue, and health knowledge.'
        : 'Healthy Diet 專案正式上線，整合飲食紀錄、AI 對話與健康資訊展示。',
    },
  ];

  const ragHighlights = [
    {
      title: t('member.rag.q1Title', 'RAG 是什麼？'),
      description: t('member.rag.q1Desc', 'RAG 是 Retrieval-Augmented Generation，中文常譯為「檢索增強生成」。它會先去找相關資料，再讓模型依據資料生成回答。'),
      icon: <BookOpen size={18} />,
    },
    {
      title: t('member.rag.q2Title', 'RAG 怎麼運作？'),
      description: t('member.rag.q2Desc', '常見流程是「問題進來 -> 檢索知識庫 -> 挑出相關內容 -> 把內容交給模型作答」，讓回答不只靠模型記憶，也能引用專案知識。'),
      icon: <Database size={18} />,
    },
    {
      title: t('member.rag.q3Title', '為什麼健康飲食系統適合用 RAG？'),
      description: t('member.rag.q3Desc', '飲食建議會碰到營養知識、專案文件與內部規則。RAG 能把這些資料集中管理，回答時更容易貼近系統脈絡與實際需求。'),
      icon: <BrainCircuit size={18} />,
    },
    {
      title: t('member.rag.q4Title', 'RAG 的限制'),
      description: t('member.rag.q4Desc', '如果知識庫內容過期、檢索不到重點，模型仍可能給出不完整答案。所以文件整理、資料更新與檢索品質同樣重要。'),
      icon: <Zap size={18} />,
    },
  ];
  ...
  const repositories = [
    {
      ...
      description: t('member.repos.web', '前端網站，負責使用者介面、互動流程與資料展示。'),
    },
    {
      name: 'healthy-diet-django',
      href: 'https://github.com/archie0732/healthy-diet-django',
      description: t('member.repos.django', 'Django 後端，提供核心 API、資料處理與系統整合能力。'),
    },
    {
      name: 'healthy-diet-ai-agent',
      href: 'https://github.com/archie0732/healthy-diet-ai-agent',
      description: t('member.repos.agent', 'AI Agent 與對話能力實作，承接推理、工具串接與知識工作流。'),
    },
    {
      name: 'healthy-diet-api',
      href: 'https://github.com/PU-Hub/healthy-diet/tree/main/healthy-diet-api',
      description: t('member.repos.api', '專案 API 相關程式碼與整合資源，補齊前後端與服務端之間的串接。'),
    },
  ];
    ...
    <div className="max-w-6xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700 px-4 sm:px-6">
      <div className="bg-slate-900 rounded-[48px] p-8 sm:p-16 text-center text-white shadow-2xl relative overflow-hidden border-b-8 border-indigo-600 mt-4 sm:mt-0">
        ...
        <div className="relative z-10 space-y-6">
          <div className="inline-flex items-center gap-2 bg-indigo-500/20 border border-indigo-500/30 px-4 py-2 rounded-full text-indigo-400 text-xs font-black uppercase tracking-widest">
            <Users size={14} /> {t('member.badge', 'The Elite Team')}
          </div>
          <h1 className="text-4xl sm:text-6xl font-black tracking-tighter bg-clip-text text-transparent bg-gradient-to-b from-white to-slate-400">
            {t('member.title', 'Healthy Diet Project Team')}
          </h1>
          <p className="text-slate-400 max-w-2xl mx-auto text-base sm:text-xl font-medium leading-relaxed">
            {t('member.subtitle', '我們把健康飲食、AI 對話、知識檢索與產品體驗放在同一個專案裡，持續打造一個更實用、更可信任的智慧飲食助理。')}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[40px] border-2 border-slate-200 p-6 sm:p-12 shadow-sm">
        <h2 className="text-2xl font-black text-slate-800 mb-8 flex items-center gap-3">
          <LayoutIcon className="text-indigo-600" /> {t('member.versionHistory', '版本歷程')}
        </h2>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-[1.1fr_0.9fr] gap-6">
        <div className="bg-white rounded-[40px] border-2 border-slate-200 p-6 sm:p-10 shadow-sm">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-2xl font-black text-slate-900">{t('member.ragSectionTitle', 'RAG 科普小教室')}</h2>
              <p className="text-sm font-bold text-slate-500">
                {t('member.ragSectionSubtitle', '讓 AI 回答不只靠模型記憶，而是能先查資料再回覆。')}
              </p>
            </div>
          </div>
        ...
        <div className="bg-slate-900 rounded-[40px] p-6 sm:p-10 shadow-2xl border-4 border-slate-800 text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-2xl font-black">{t('member.githubTitle', '專案 GitHub')}</h2>
                <p className="text-sm font-bold text-slate-400">
                  {t('member.githubSubtitle', '這裡可以看到前端、後端、AI Agent 與 API 相關程式碼。')}
                </p>
              </div>
            </div>
      ...
      <div className="bg-slate-900 rounded-[40px] p-8 sm:p-12 flex flex-col md:flex-row md:items-center justify-between shadow-2xl border-4 border-slate-800 relative overflow-hidden group">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-6 mb-8 md:mb-0 relative z-10">
          ...
          <div>
            <h3 className="text-2xl sm:text-3xl font-black text-white mb-2">{t('member.devApiTitle', 'Developers API')}</h3>
            <p className="text-slate-400 text-sm font-bold max-w-sm">
              {t('member.devApiSubtitle', '想進一步了解系統串接方式、端點設計與資料流，可以從 API 文件快速掌握整體架構。')}
            </p>
          </div>
        </div>
        <Link
        ...
        >
          <Code size={20} className="stroke-[2.5px]" /> {t('member.devApiButton', '查看 API 文件')}
        </Link>
      </div>
      ...
      <div className="flex flex-col items-center gap-6 pt-6">
        ...
        >
          ...
          {t('member.privacyButton', '查看隱私權政策與服務條款')}
        ...
        <p className="text-[10px] sm:text-[11px] text-slate-400 font-bold uppercase tracking-widest text-center">
          {t('member.copyright', '© 2026 Healthy Diet Project Team. All Rights Reserved.')}
        </p>
      </div>
    </div>
  );
